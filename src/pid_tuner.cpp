...
WebControlPanel::WebControlPanel()
    : server(nullptr), pitchPID(nullptr), rollPID(nullptr), wifiEnabled(false),
      kp_pitch(1), ki_pitch(0), kd_pitch(0), kp_roll(1), ki_roll(0), kd_roll(0) {}
...
void WebControlPanel::loadPIDGains(float default_kp_pitch, float default_ki_pitch, float default_kd_pitch,
                                   float default_kp_roll, float default_ki_roll, float default_kd_roll) {
...
void WebControlPanel::setupWebHandlers() {
  ...
  server->on("/update", HTTP_POST, [this](AsyncWebServerRequest *request){
    if (request->hasParam("kp_pitch", true)) kp_pitch = request->getParam("kp_pitch", true)->value().toFloat();
    if (request->hasParam("ki_pitch", true)) ki_pitch = request->getParam("ki_pitch", true)->value().toFloat();
    if (request->hasParam("kd_pitch", true)) kd_pitch = request->getParam("kd_pitch", true)->value().toFloat();
    if (request->hasParam("kp_roll", true)) kp_roll = request->getParam("kp_roll", true)->value().toFloat();
    if (request->hasParam("ki_roll", true)) ki_roll = request->getParam("ki_roll", true)->value().toFloat();
    if (request->hasParam("kd_roll", true)) kd_roll = request->getParam("kd_roll", true)->value().toFloat();
...
void WebControlPanel::updatePIDGains() {
  if (pitchPID) pitchPID->setGains(kp_pitch, ki_pitch, kd_pitch);
  if (rollPID) rollPID->setGains(kp_roll, ki_roll, kd_roll);
}
...
String WebControlPanel::buildWebPage() {
  ...
  html += ".section { margin: 20px 0; padding: 10px; border: 1px solid #ddd; background: white; }";
...
String WebControlPanel::buildPIDTuningSection() {
  ...
  html += "Kp: <input type='number' step='0.01' name='kp_pitch' value='" + String(kp_pitch, 2) + "'><br>";
  html += "Ki: <input type='number' step='0.01' name='ki_pitch' value='" + String(ki_pitch, 2) + "'><br>";
  html += "Kd: <input type='number' step='0.01' name='kd_pitch' value='" + String(kd_pitch, 2) + "'><br>";
  html += "</div>";
  
  html += "<div class='section'><h3>Roll PID</h3>";
  html += "Kp: <input type='number' step='0.01' name='kp_roll' value='" + String(kp_roll, 2) + "'><br>";
  html += "Ki: <input type='number' step='0.01' name='ki_roll' value='" + String(ki_roll, 2) + "'><br>";
  html += "Kd: <input type='number' step='0.01' name='kd_roll' value='" + String(kd_roll, 2) + "'><br>";
  html += "</div>";
